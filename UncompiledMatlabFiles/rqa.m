...
set(handles.cbRPMatrix,'Enable','On');
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[FileName2,PathName2] = uigetfile('*.txt','Select a file',handles.pathname);
if ~FileName2 
    return
end
...
if handles.xRQA == 1
    ...
    cla;
    axes(handles.axTS1);
    plot(handles.data1, 1:length(handles.data1), 'b');
    ylim([1 length(handles.data1)]);
else
    ...
    axes(handles.axTS2);
    plot(handles.data1, 'b');
    xlim([1 length(handles.data1)]);
end
...
data1filename = get(handles.edFileName, 'String');
data2filename = get(handles.edFileName2, 'String');
rescale = handles.rescale;
normalize = handles.normalize;
mll = str2double(get(handles.etMinLineLength, 'String'));
delay = str2double(get(handles.edTLag, 'String'));
...
if exist(outfilename, 'file')==2
    ...
    if (handles.xRQA == 1)
        fprintf(fout,'%s, %s,',data1filename, data2filename);
    else
        fprintf(fout,'%s, N/A,',data1filename);
    end
...
function pbDelay_Callback(hObject, eventdata, handles)
...
if handles.CatRQA == 1
    ...
    set(handles.pbRadius,'Enable','Off');
    clearGUI(handles)
else
    ...
    set(handles.pbRadius,'Enable','On');
    clearGUI(handles);
end
